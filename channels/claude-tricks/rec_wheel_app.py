#!/usr/bin/env python3
"""Capture the Spin-The-Wheel decider demo — franchise entry #3.

WHY THIS TOOL EXISTS (2026-08-19 QC finding): tip #3's cut failed frame-by-frame
QC because ~14 of 39s were FROZEN SCREENSHOTS sitting in the 3-15s sustain window.
The habit-tracker short had the same latent flaw (its payoff was a baked
still-hold) because mobile tap-to-open was flaky. That is now SOLVED — the real
clickable is `button[aria-label^="View "]` — so this tool films the artifact
OPENED AND RUNNING: the wheel actually spins on camera. Continuous motion is the
direct lever on the Shorts-feed-selection ceiling (the metric that caps us).

One take, mobile 540x960@2 = 1080x1920:
  type one line -> Claude builds -> artifact card -> OPEN fullscreen ->
  tap SPIN -> the wheel spins (real motion) -> lands on a result -> hold.
...
ASSETS = ROOT / "channels/claude-tricks/assets/ep_wheel"
VIEW = {"width": 540, "height": 960}
UA = ("Mozilla/5.0 (iPhone; CPU iPhone OS 17_5 like Mac OS X) "
      "AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.5 Mobile/15E148 Safari/604.1")

# ONE short line so "type one line" is literal on camera. Asking for a big
# centered SPIN button gives a single, predictable tap target to film.
PROMPT = ("Build me a spin the wheel app to decide what to eat - 6 dinner options, "
          "big colorful wheel, one big SPIN button in the middle.")
TITLE_CANDS = ["Spin the wheel", "Spin The Wheel", "Dinner spinner", "Dinner Spinner",
               "What to eat", "What To Eat", "Wheel spinner", "Wheel Spinner",
               "Dinner wheel", "Dinner Wheel"]
...
def main():
    ...
    ap.add_argument("--spins", type=int, default=2, help="how many times to tap SPIN on camera")
    a = ap.parse_args()
    ...
    with sync_playwright() as p:
        ...
        def find_spin():
            ...
            try:
                ...
                for name in ("SPIN", "Spin", "spin", "SPIN!"):
                    c = fr.get_by_role("button", name=name).first
                    if c.count():
                        return c, f"iframe-role:{name}"
                c = fr.get_by_text("SPIN", exact=True).first
                if c.count():
                    return c, "iframe-text:SPIN"
            except Exception as e:
                print(f"iframe spin lookup: {e!r}")
        ...
        for attempt in range(3):
            ...
            spin, how = find_spin()
            if spin is not None:
                print(f"APP IS OPEN — spin control via {how}"); break
        ...
        CX, CY = VIEW["width"] // 2, int(VIEW["height"] * 0.502)   # SPIN hub (from still_open)
        for i in range(a.spins):
            before = None
            try:
            ...
            try:                                        # did anything actually move?
                ...
                pass
            time.sleep(7.0)                             # let the spin play out + land, on camera
        time.sleep(2.5)                                 # hold the winning result
