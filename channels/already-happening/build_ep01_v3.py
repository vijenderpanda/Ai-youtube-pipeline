...
MOT = os.path.join(HERE, "assets", "ep01", "motion_v3")
VO = os.path.join(HERE, "renders", "ep01", "vo_v3")
OUT = os.path.join(HERE, "renders", "ep01")
TMP = tempfile.mkdtemp(prefix="ah_v3_")
...
BEATS = [
    ("first-person-pov-driving-forward_1f195563.mp4", "beat1"),   # S1 hook (night POV)
    ("cinematic-documentary-shot-a-sle_1f195579.mp4", "beat2"),   # S2 tell (robotaxi)
    ("cinematic-tracking-shot-followin_1f19557d.mp4",  "beat3"),  # S3 today (tracking)
    ("cinematic-over-the-shoulder-shot_1f195588.mp4",  "beat4"),  # S4 NEW booking
    ("cinematic-slow-push-in-through-a_1f195582.mp4",  "beat5"),  # S5 payoff (park)
    ("cinematic-first-person-pov-insid_1f195585.mp4",  "beat6"),  # S6 button (wheel)
]
HOT = {"FIFTY","THOUSAND","NO","DRIVER","WEEK","TEST","TUESDAY","TODAY","PHOENIX",
       "FRANCISCO","AUSTIN","NOBODY","WHEEL","TAP","PHONE","NINETY","SECONDS","POD",
       "CHEAPER","THIRD","PARKING","PARKS","HOMES","STREETS","AI","ROUTE","SPEED",
       "GIVE","UP","OWN","ONE"}
...
def main():
    ...
    out=os.path.join(OUT,"ep01_v3.mp4")
    total=COLD+sum(durs)+2.6
    ...
    print(f"OK -> {out}  (~{total:.1f}s, bed=bed_3.mp3)")
