...
#
#   .\install.ps1 -NoService     # set up config + deps only, no autostart task
#
...
if (-not $claude) {
  Die "'claude' not found on PATH. Install Claude Code for Windows and sign in first:
...
if (Test-Path .env) { Ok ".env already exists — leaving it untouched" }
else { Copy-Item deploy\api-keys.env.example .env; Ok "Created .env for generation API keys (optional)" }
...
if ($LASTEXITCODE -eq 0) { Ok "worker dry-run OK" }
else { Warn "dry-run issue — see $env:TEMP\factory_dry.log (usually an unfilled secrets\factory.env)" }
