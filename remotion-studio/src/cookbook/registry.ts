...
export const COOKBOOK: CookbookEntry[] = [
  ...
  {
    id: "PhoneMock", demoId: "PhoneMockDemo", title: "Phone chat-thread mockup",
    role: "device-ui", beats: ["demo", "process", "punchline"], needs: "dialogue",
    keywords: ["phone", "iphone", "imessage", "chat", "thread", "bubbles", "messaging", "mockup", "device", "typing", "sms", "whatsapp"],
    transparentCapable: true, wow: 3, density: "med",
    useWhen: "A text/chat exchange happening inside an app on a phone — command→reply proof, assistant-in-your-messages demos.",
    gist: "Tilted 9:19.5 phone frame (island + status bar) holding a bottom-anchored thread; grey incoming / accent outgoing bubbles land on a schedule with typing dots, then hold. Cream-first (comp-dna), retintable. Invented thread — never a real app screenshot.",
  },
  {
    id: "FlowTree", demoId: "FlowTreeDemo", title: "Node tree / flowchart",
    role: "dataviz", beats: ["process", "context", "demo"], needs: "steps",
    keywords: ["tree", "flowchart", "branch", "agents", "sub-tasks", "workflow", "plan", "breakdown", "hierarchy", "decision path", "pipeline", "parent child"],
    transparentCapable: true, wow: 4, density: "med",
    useWhen: "Break a system/plan into parts or show a branching workflow — connectors draw, children pop in, the active path lights in accent.",
    gist: "Root-down (or left-right) node tree, ≤7 nodes/3 levels; right-angle connectors draw via dashoffset before each child pops; done=✓, active=accent fill + arrival ring, todo=dashed mute. Cream theme uses comp-dna dark-green pills. A diagram of structure, never a live-product claim.",
  },
  {
    id: "StatCloser", demoId: "StatCloserDemo", title: "Editorial number closer",
    role: "dataviz", beats: ["stat", "punchline", "social-proof"], needs: "single-number",
    keywords: ["stat", "number", "closer", "count-up", "figure", "editorial", "cream", "card", "total", "payoff", "compare", "source", "saved", "spent"],
    transparentCapable: true, wow: 3, density: "low",
    useWhen: "Land ONE supplied figure as the calm editorial payoff/closer — count-up + accent underline on a cream card, optional struck compare + source chip. Not for reel drama (Odometer) or dark glass (GlassPanel).",
    gist: "Cream editorial number-closer: small-caps label, Anton figure counts up and locks with a settle kick, accent underline sweeps, sans sub with serif-italic punch, optional ghost compare + source chip; holds.",
  },
  {
    id: "ChipRow", demoId: "ChipRowDemo", title: "Pill / chip row",
    role: "layout", beats: ["context", "comparison", "demo"], needs: "facts",
    keywords: ["chip", "pill", "tag", "badge", "toggle", "objection", "strike", "model", "platform", "row", "labels", "supports", "works with"],
    transparentCapable: true, wow: 2, density: "med",
    useWhen: "3-8 short labels must land one at a time on VO — platforms/models supported, objections crossed out, a setting toggled, options with one emphasised. Transparent overlay over host/tape is the primary use.",
    gist: "A row (or wrapped rows) of rounded chips popping in staggered with overshoot+settle; kinds label/model/platform/objection/toggle; any chip can go hot (accent fill) or struck (accent strike draws, dims, serif-italic); optional eyebrow, icon, per-chip t. Cream default, brand retint.",
  },
  {
    id: "BrandBumper", demoId: "BrandBumperDemo", title: "Brand / product bumper",
    role: "typography", beats: ["hook", "cta"], needs: "phrase",
    keywords: ["bumper", "brand", "logo", "wordmark", "starburst", "ring-burst", "intro", "outro", "sting", "cta", "orbit", "product reveal"],
    transparentCapable: true, wow: 3, density: "low",
    useWhen: "A 1.2-2s brand/product reveal at the open or close: glyph/emoji lands on a radiating burst, wordmark types in, optional tagline; close mode adds a CTA pill + pulse. Glyph enum + text only — never a real third-party logo.",
    gist: "Icon-on-starburst bumper card with typed wordmark, tagline, and open/close CTA variant (cream default, comp-dna H-logo-motion hook).",
  },
  {
    id: "SplitHead", demoId: "SplitHeadDemo", title: "Split-register title card",
    role: "typography", beats: ["hook", "context", "punchline", "cta"], needs: "phrase",
    keywords: ["title card", "headline", "chapter", "serif italic", "punch word", "editorial", "split register", "cream", "label", "statement", "how to"],
    transparentCapable: true, wow: 3, density: "low",
    useWhen: "A structural pivot or statement beat on the cream canvas: label + headline where ONE punch word carries the claim (chapter opens, 'HOW TO' cards, editorial claims). Not a caption (SerifCap), not the brand-dark punchline (KineticQuote).",
    gist: "Cream title card: small heavy sans label over a big headline whose ONE punch word flips to Playfair italic in accent; words land as rise+settle clusters, punch last, accent highlight-bar wipes under it; optional kicker + muted sub; auto-wraps ≤3 rows.",
  },
  {
    id: "AppWindow", demoId: "AppWindowDemo", title: "Desktop app-window mockup",
    role: "app-ui", beats: ["demo", "context", "process"], needs: "facts",
    keywords: ["app window", "mockup", "fake app", "code editor", "note app", "desktop", "sidebar", "table", "typed lines", "software", "editor", "notes", "vault"],
    transparentCapable: true, wow: 3, density: "med",
    useWhen: "An invented desktop app / editor / note-list / mini-table mockup with typed or staggered content. NOT for real-product footage (ScreenStage/WebTour) or pure terminal (TermRun).",
    gist: "Desktop window chrome (traffic lights, title, optional URL bar + sidebar) dropping in with mass; body types on lines/code or lands table rows staggered, caret blinks, then holds with idle drift. Light note-app or dark editor chrome on cream/brand canvas.",
  },
];
